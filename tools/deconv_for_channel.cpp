...
template <typename Dtype>
int ResetBlob(shared_ptr< Blob<Dtype> >& data_blob){
	Dtype *feature_diff = data_blob->mutable_cpu_diff();
	caffe_set(data_blob->count(), Dtype(0), feature_diff);
}

template <typename Dtype>
int SetChannel(shared_ptr< Blob<Dtype> >& data_blob, int channelIdx){
	int num = data_blob->num();
	int channels = data_blob->channels();
	int height = data_blob->height();
	int width = data_blob->width();
	...
	for(int n=0;n<num;++n){
		...
		if(channelIdx>-1){
			for(int h=0;h<height;++h){
				for(int w=0;w<width;++w){
					int pixelIdx = (channelIdx * height + h ) * width + w;
					feature_diff[pixelIdx]= feature_data[pixelIdx];
					//feature_diff[pixelIdx]= Dtype(0);
				}
			}
...
int ex_feature(int argc, char** argv){
	...
	if (argc < 10){
		LOG(ERROR)<< "Usage: "<<argv[0]<<" pretrained_net_param feature_extraction_proto_file extract_feature_blob_name start_channel end_channel filelist meanfile savefile mode";
		return 1;
	}
	...
	int start_channel = atoi(argv[4]);
	int end_channel = atoi(argv[5]);
	string tst_filelist=argv[6];
	string mean_file = argv[7];
	...
	for(int cacheIdx = 0;cacheIdx < numCaches;cacheIdx++){
		...
		for(int batchIdx = 0;batchIdx < nBatches; batchIdx++){
			...
			int channels_t = feature_blob->channels();
			LOG(ERROR) << "feature_blob:" << height_t << " " << width_t;
			ResetBlob(feature_blob);
			feature_extraction_net->ReSetActions(layerIdx);
			SetChannel(feature_blob, -1);
			...
			for(int s=0;s<n;++s){
					...
					show_reconstruct_image<Dtype>(feature_blob_data, im_blob_ori, data_blob->count() / data_blob->num(),
						data_blob->height(), data_blob->width(), data_blob->channels());
			}
			for(int cc = start_channel;cc <= end_channel;cc++){
				//for(int ww = 0;ww<width_t;++ww){
					LOG(ERROR) << "Reconstructing channel " << cc;
					ResetBlob(feature_blob);
					feature_extraction_net->ReSetActions(layerIdx);
					SetChannel(feature_blob, cc);
					//SetTopAct<Dtype>(feature_blob);
					feature_extraction_net->Deconv(layerIdx);
					//return 1;
