...
template <typename Dtype>
int SetTop(shared_ptr< Blob<Dtype> >& data_blob, int xx, int yy, int cc){
	int num = data_blob->num();
	int channels = data_blob->channels();
	int height = data_blob->height();
	int width = data_blob->width();
	...
	for(int n=0;n<num;++n){
		...
		if(cc > -1){
			int pixelIdx = (cc * height + xx) * width + yy;
			feature_diff[pixelIdx]=feature_data [pixelIdx];
		}else{
			int maxc=0;
			Dtype maxvalue=feature_data[xx*width+yy];
			for(int c=0;c<channels;c++){
				int pixelIdx = (c * height + xx) * width + yy;
				//feature_diff[pixelIdx] = feature_data[pixelIdx];
				if(maxvalue < feature_data[pixelIdx]){
					maxvalue = feature_data[pixelIdx];
					maxc = c;
				}
			}
			feature_diff[(maxc*height+xx)*width + yy] = maxvalue;
		}
	}
...
template <typename Dtype>
int get_reconstruct_image(Dtype* feature_blob_data,const Dtype* ori, int feature_dim, int height, int width, int channels, cv::Mat& im_ret, 
	cv::Mat& im_ori_ret, cv::Mat& im_rectify_ret){
	...
	//cv::namedWindow("reconstruct");
	//cv::namedWindow("ori");
	//cv::namedWindow("reconstruct_rectify");
	//cv::namedWindow("equalized_im");
	//cv::imshow("reconstruct", im);
	//cv::waitKey(100);
	//cv::imshow("ori", im_ori);
	//cv::waitKey(100);
	//cv::equalizeHist(im_rectify, im_rectify);
	//cv::imshow("reconstruct_rectify", im_rectify);
	//cv::waitKey(100);
	//cv::imshow("equalized_im", equalized_im);
	//cv::waitKey(100);
	//cv::destroyAllWindows();
	/*int tot_height = im_ori.rows*2;
	int tot_width = im_ori.cols*2;
	cv::Mat im_show(tot_height, tot_width, CV_32FC3);
	
	for(int c=0;c<channels;c++){
		for(int h=0;h<height;h++){
			for(int w=0;w<width;w++){
				im_show.at<cv::Vec3f>(h,w)[c] =float( im_ori.at<cv::Vec3b>(h,w)[c]);
				im_show.at<cv::Vec3f>(h+height,w)[c] = float(im_rectify.at<cv::Vec3b>(h,w)[c]);
				im_show.at<cv::Vec3f>(h, w+width)[c] = float(im.at<cv::Vec3f>(h,w)[c]);
				im_show.at<cv::Vec3f>(h+height, w+w+width)[c]=float(equalized_im.at<cv::Vec3b>(h,w)[c]);
			}
		}
	}*/
	//cv::namedWindow("deconv", CV_WINDOW_NORMAL);
	//cv::resizeWindow("deconv", 480, 960);
	//cv::Mat im_resized;
	//cv::resize(im_show, im_resized, cv::Size(0,0), 0.5, 0.5);
	//LOG(ERROR) << "shape of im_show, height:" << im_show.rows << " width:" << im_show.cols; 
	//cv::imshow("deconv", im_resized);
...
template <typename Dtype>
void show_reconstruct_image_for_position(shared_ptr<Net<Dtype> >& deconvNet, shared_ptr<Blob<Dtype> >& feature_blob, 
	shared_ptr<Blob<Dtype> >& data_blob, int layerIdx, int n){
	//int hh=0,ww=0,cc=0;
	cv::Mat im(1, 1, CV_32FC3), im_ori(1,1, CV_8UC3), im_rectify(1,1, CV_8UC3);
	//cv::namedWindow("reconstruct");
	//cv::namedWindow("ori");
	//cv::namedWindow("reconstruct_rectify");
	int height = data_blob->height();
	...
	for(int hh=0;hh<height;++hh){
		for(int ww=0;ww<width;++ww){
			//std::cin >> ww >> cc;
			deconvNet->ReSetActions(layerIdx);
			SetTop(feature_blob, hh, ww, -1);
			deconvNet->Deconv(layerIdx);
			//LOG(ERROR) << "deconv done";
			for(int s=0;s<n;++s){
				Dtype* feature_blob_data = data_blob->mutable_cpu_diff()+data_blob->offset(s);
				const Dtype* im_blob_ori = data_blob->cpu_data() + data_blob->offset(s);
				get_reconstruct_image<Dtype>(feature_blob_data, im_blob_ori, data_blob->count() / data_blob->num(), data_blob->height(), 
					data_blob->width(), data_blob->channels(), im, im_ori, im_rectify);	
				cv::imshow("reconstruct", im);
				cv::imshow("ori", im_ori);
				//cv::imshow("reconstruct_rectify", im_rectify);
				cv::waitKey(0);
