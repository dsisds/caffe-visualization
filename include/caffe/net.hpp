...
class Net {
 public:
  ...
  const vector<Blob<Dtype>*>& ForwardPrefilled(int layerIdx, Dtype* loss = NULL);
  // Run forward using a set of bottom blobs, and return the result.
  const vector<Blob<Dtype>*>& Forward(const vector<Blob<Dtype>* > & bottom,
      Dtype* loss = NULL);
  const vector<Blob<Dtype>*>& Forward(const vector<Blob<Dtype>* >& bottom, int layerIdx, 
	  Dtype* loss = NULL);
  ...
  void Deconv(int index);
  void ReSetActions(int index);

  void ResetLayers(int index, int height, int width);
  ...
  int layerIdx_by_name(const string& layer_name);
