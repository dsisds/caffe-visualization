...
template <typename Dtype>
void Net<Dtype>::ResetLayers(int layerIdx, int height, int width){
	if(layerIdx <= 0){
		return;
	}
	assert(layerIdx < layers_.size());
	vector<Blob<Dtype>*>* bottom = &bottom_vecs_[0];
	int num_images = (*bottom)[0]->num();
	assert(num_images == 1);
	if(((*bottom)[0]->height() == height) && ((*bottom)[0]->width() == width)){
		return;
	}
	int channels = (*bottom)[0]->channels();
	(*bottom)[0]->Reshape(num_images, channels, height, width);
...
template <typename Dtype>
const vector<Blob<Dtype>*>& Net<Dtype>::ForwardPrefilled( int layerIdx, Dtype* loss) {
  if (loss != NULL) {
    *loss = Dtype(0.);
  }
  assert(layerIdx < layers_.size());
...
template <typename Dtype>
const vector<Blob<Dtype>*>& Net<Dtype>::Forward(
    const vector<Blob<Dtype>*> & bottom, int layerIdx, Dtype* loss) {
  // Copy bottom to internal bottom
  for (int i = 0; i < bottom.size(); ++i) {
    net_input_blobs_[i]->CopyFrom(*bottom[i]);
  }
  return ForwardPrefilled(layerIdx, loss);
}
...
template <typename Dtype>
void Net<Dtype>::SetTop(int index){
	vector<Blob<Dtype>*>* bottom = &top_vecs_[index];
	int num = (*bottom)[0]->num();
	int channels = (*bottom)[0]->channels();
	int height = (*bottom)[0]->height();
	int width = (*bottom)[0]->width();
	LOG(ERROR) << "blob:num" << num << " channels:" << channels << " height:" << height << " width:" << width; 
...
template <typename Dtype>
void Net<Dtype>::Deconv(int index){
	if(index <=0){
		return;
	}
	//layers_[index]->SetTop(&top_vecs_[index]);
	//SetTop(index);
	for(int i=index;i>=0;--i){
		LOG(ERROR) << "reconstruct Layer:" << layers_[i]->type_name();
		layers_[i]->Reconstruct(top_vecs_[i], &bottom_vecs_[i]);
	}
}

template <typename Dtype>
void Net<Dtype>::ReSetActions(int index){
	if(index <=0){
		return;
	}
	for(int i=index;i>=0;--i){
		LOG(ERROR) << "Reset Layer:" << layers_[i]->type_name();
		//layers_[i]->Reconstruct(top_vecs_[i], &bottom_vecs_[i]);
		vector<Blob<Dtype>*>* bottom = &bottom_vecs_[i];
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i){
      LOG(ERROR) << "Source Layer:" << param.layers(i).name();
  }
  for (int i = 0; i < num_source_layers; ++i) {
...
template <typename Dtype>
int Net<Dtype>::layerIdx_by_name(
    const string& layer_name) {
  if (has_layer(layer_name)) {
    return layer_names_index_[layer_name];
  } else {
