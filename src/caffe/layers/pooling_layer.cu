...
template <typename Dtype>
__global__ void MaxPoolReconstruct(const int nthreads, const Dtype* top_diff,
    const int* mask, const Dtype* top_mask, const int num, const int channels,
    const int height, const int width, const int pooled_height,
    const int pooled_width, const int kernel_h, const int kernel_w,
    const int stride_h, const int stride_w, const int pad_h, const int pad_w,
...
template <typename Dtype>
__global__ void AvePoolReconstruct(const int nthreads, const Dtype* top_diff,
    const int num, const int channels, const int height,
    const int width, const int pooled_height, const int pooled_width,
    const int kernel_h, const int kernel_w, const int stride_h,
    const int stride_w, const int pad_h, const int pad_w,
...
template <typename Dtype>
void PoolingLayer<Dtype>::Reconstruct_gpu(const vector<Blob<Dtype>*>& top,
  ...
  case PoolingParameter_PoolMethod_MAX:
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    MaxPoolReconstruct<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, top_diff, mask, top_mask, top[0]->num(), channels_,
        height_, width_, pooled_height_, pooled_width_,
        kernel_h_, kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_,
        bottom_diff);
  ...
  case PoolingParameter_PoolMethod_AVE:
    // NOLINT_NEXT_LINE(whitespace/operators)
    AvePoolReconstruct<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, top_diff, top[0]->num(), channels_,
        height_, width_, pooled_height_, pooled_width_, kernel_h_,
        kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_, bottom_diff);
    break;
...
/*
template <typename Dtype>
void PoolingLayer<Dtype>::SetTop_cpu(vector<Blob<Dtype>*>* bottom){
	...
	//for(int n=0;n<num;++n){
		const Dtype* feature_data = (*bottom)[0]->gpu_data()+(*bottom)[0]->offset(n);
		Dtype* feature_diff = (*bottom)[0]->mutable_gpu_diff()+(*bottom)[0]->offset(n);
		int x=0, y=0, ch=0;
