...
template <typename Dtype>
Dtype LRNLayer<Dtype>::CrossChannelReconstruct_cpu(const vector<Blob<Dtype>*>& top, 
		vector<Blob<Dtype>*>* bottom) {
  const Dtype* top_diff = top[0]->cpu_diff();
  Dtype* bottom_diff = (*bottom)[0]->mutable_cpu_diff();
  Dtype* scale_data = scale_.mutable_cpu_data();
  ...
  // In the end, compute output
  caffe_powx<Dtype>(scale_.count(), scale_data, -beta_, bottom_diff);
  caffe_mul<Dtype>(scale_.count(), bottom_diff, top_diff, bottom_diff);
...
/*
template <typename Dtype>
Dtype LRNLayer<Dtype>::WithinChannelReconstruct(
  const vector<Blob<Dtype>*>& top, vector<Blob<Dtype>*>* bottom) {
  split_layer_->Forward(top, &split_top_vec_);
  square_layer_->Forward(square_bottom_vec_, &square_top_vec_);
  pool_layer_->Forward(square_top_vec_, &pool_top_vec_);
  power_layer_->Forward(pool_top_vec_, &power_top_vec_);
  product_layer_->Forward(product_bottom_vec_, top);
...
template <typename Dtype>
void LRNLayer<Dtype>::Reconstruct_cpu(const vector<Blob<Dtype>*>& top,
    vector<Blob<Dtype>*>* bottom) {
  switch (this->layer_param_.lrn_param().norm_region()) {
  case LRNParameter_NormRegion_ACROSS_CHANNELS:
    CrossChannelReconstruct_cpu(top, bottom);
    break;
  case LRNParameter_NormRegion_WITHIN_CHANNEL:
    NOT_IMPLEMENTED;
    break;
...
template <typename Dtype>
void LRNLayer<Dtype>::SetTop_cpu(vector<Blob<Dtype>*>* bottom){
