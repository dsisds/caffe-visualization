...
template <typename Dtype>
void ConvolutionLayer<Dtype>::Reconstruct_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < top.size(); ++i) {
	  ...
	  /*if(bias_term_){
		...
		bias_multiplier_.reset(new Blob<Dtype>(1, 1, 1, N));
		caffe_set(N, Dtype(1), bias_multiplier_->mutable_cpu_data());
	  }*/
	  Blob<Dtype>*  top_t= new Blob<Dtype>(top[i]->num(), top[i]->channels(), top[i]->height(), top[i]->width());
	  Dtype* top_t_data = top_t->mutable_cpu_data();
	  for(int k=0;k<top[i]->count();++k){
		  top_t_data[k] = top_diff[k];
	  }
	  ...
	  for (int n = 0; n < num_; ++n) {
		  ...
		  /*if (bias_term_) {
			caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_,
            height_*width_, 1, (Dtype)-1., this->blobs_[1]->cpu_data(),
            bias_multiplier_->cpu_data(),
            (Dtype)1., bottom_diff + (*bottom)[i]->offset(n));
		  }*/
	  }
	  delete top_t;
  }
...
template <typename Dtype>
void ConvolutionLayer<Dtype>::SetTop_cpu(vector<Blob<Dtype>*>* bottom){
